...
from src.models import GameIn, ReferenceIn, CollectionIn, GameTypeEnum
...
def create_game_bools(game: db.Game, request: GameIn):
...
def update_game_relationships(game: db.Game, request: GameIn):
...
def create_game_relationships(game: db.Game, request: GameIn):
...
def create_game_unique(game: db.Game, request: GameIn):
...
def create_game_meta(game: db.Game, request: GameIn):
    """
    """
    db.GameMeta(
        game=game,
...
def create_games(games: List[GameIn]):
    ...
    for game in games:
        try:
            new_instance = db.Game(license=db.License(**game.license.dict()))
