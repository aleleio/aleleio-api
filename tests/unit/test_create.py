...
from src.services.create import slugify, set_game_bools, set_game_categories, set_game_group_needs
...
def test_create_game_relationships(db):
    ...
    set_game_categories(game, request)
...
def test_create_game_relationships_with_group_needs(db):
    game = db.Game(license=db.License())
    request = {"group_needs": [{"slug": "first", "score": 5}, {"slug": "honesty", "score": 0}]}
    set_game_group_needs(game, request)
